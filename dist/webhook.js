import { getGitlabUser, getIssue, getIssueComments, postIssueComment, updateIssue, deleteIssueComment } from "./gitlab.js";
...
export async function initBotUser() {
    try {
        ...
        logger.error("Warning: Could not fetch GitLab bot username: " + err.message);
    }
}
...
function verifyGitlabSignature(id, timestamp, rawBody, signatureHeader, signingToken) {
    try {
        ...
        logger.error("Error verifying GitLab webhook signature: " + err.message);
        return false;
    }
}
function isBotMessage(payload) {
    if (payload.object_kind === "note") {
        ...
        return (noteText.includes("Proposal from @dev-assist") ||
            noteText.includes("thanks for opening this issue!"));
    }
    return false;
}
export async function handleGitlabWebhook(req, res, opencodeClient) {
...
async function runPublishCommand(projectId, issueIid, res) {
    ...
    if (!titleMatch || !descMatch) {
        ...
        await postIssueComment(projectId, issueIid, errMsg);
        if (res)
            res.status(400).json({ error: "Failed to parse proposal" });
        return;
    }
