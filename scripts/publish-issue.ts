...
// Usage examples:
//   npx tsx scripts/publish-issue.ts 82888215/7
//   npx tsx scripts/publish-issue.ts 82888215/7 close,ready   (will close + add labels)
//
...
import { publishIssue } from '../src/services/processing/publisher';
...
if (!arg || !arg.includes('/')) {
  ...
  logger.error('  e.g.  npx tsx scripts/publish-issue.ts 82888215/7');
  logger.error('  e.g.  npx tsx scripts/publish-issue.ts 82888215/7 close,ready');
  process.exit(1);
}
