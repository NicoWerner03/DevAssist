...
Der finale Prototyp bildet diesen Kernworkflow technisch weitgehend ab. Ein Issue oder Kommentar mit \texttt{@dev-assist} kann einen Process-Lauf auslösen. Der Processor lädt verfügbaren Kontext und entscheidet nach der Analyse zwischen Klärungskommentar und vollständigem Vorschlag. Der Vorschlag enthält einen getrennten Titel sowie vier sichtbare Abschnitte für Beschreibung, Akzeptanzkriterien, technischen Kontext und Lösungsvorschlag; verbleibende offene Fragen werden sichtbar gehalten. \path{context.md} und \path{context.json} persistieren Beschreibung und Titel. Erst \texttt{@dev-assist publish} beziehungsweise ein manueller Publish-Aufruf aktualisiert das Issue.

Die Qualitätssicherung bestätigt die technische Stabilität wichtiger deterministischer Komponenten. Der TypeScript-Build ist erfolgreich, und 71 automatisierte Tests in 17 Suiten bestehen; für die geladenen Quelldateien werden 84,10\,\% Zeilenabdeckung erreicht. Besonders abgesichert sind das aktuelle Kompaktschema, die Ausgabe im Markdown-Format, die Aktivierungsregel, das dokumentierte GitLab-Signing-Token-Format mit Replay-Schutz, Pfad- und API-Sicherheitsgrenzen, die sichere Publish-Reihenfolge, Kommentarfilter, beantwortete Rückfragen, Zusammenfassungen des Repositorys und OpenCode-Fallbacks. Als wesentliche Lücken bleiben das gegenüber Kapitel 3 kompaktere Zielschema und fehlende vollständige positive Process-/Publish-Ende-zu-Ende-Tests.
...
\subsection{Erforderliche Informationen für umsetzbare und überprüfbare GitLab-Tickets}
...
\item \textbf{Eindeutiger Gegenstand.} Ein prägnanter Titel muss benennen, welche Funktion, Änderung oder Störung behandelt wird.
\item \textbf{Problem beziehungsweise Ziel.} Die Beschreibung muss erklären, welcher Bedarf besteht, welcher Ist-Zustand problematisch ist oder welches Ergebnis erreicht werden soll.
\item \textbf{Fachlicher Kontext und Abgrenzung.} Betroffene Rollen, Systeme, Abläufe, Voraussetzungen und bewusste Nicht-Ziele müssen erkennbar sein, soweit sie für die Entscheidung relevant sind.
\item \textbf{Erwartetes und überprüfbares Verhalten.} Akzeptanzkriterien müssen beschreiben, woran eine korrekte Umsetzung erkannt werden kann. Bei Fehlern gehören insbesondere beobachtetes Verhalten, erwartetes Verhalten und nachvollziehbare Reproduktionsschritte dazu.
\item \textbf{Technische Evidenz und Randbedingungen.} Relevante Logs, Fehlermeldungen, Plattformangaben, Abhängigkeiten oder belegte Repository-Hinweise müssen verfügbar sein, ohne fachliche Ziele durch technische Annahmen zu ersetzen.
\item \textbf{Unsicherheit und offene Entscheidungen.} Fehlende Angaben, Annahmen, Widersprüche und offene Fragen müssen sichtbar bleiben, statt durch plausiblen Zusatztext verdeckt zu werden.
\end{enumerate}

Der vom Prototyp verwendete Kompaktvertrag bildet diese Mindeststruktur teilweise ab. Die Felder \texttt{title} und \texttt{description} trennen Gegenstand und Zielbeschreibung. \texttt{acceptanceCriteria} enthält die Prüfkriterien, während \texttt{technicalContext} technische Hinweise erfasst. Das Feld \texttt{proposedSolution} hält einen vorläufigen Lösungsvorschlag fest. Verbleibende Unsicherheit steht in \texttt{openQuestions}. Der Lösungsvorschlag ist für die Übergabe hilfreich, gehört aber nicht in gleicher Weise zum fachlichen Mindestbestand wie Ziel und Akzeptanzkriterien. Er sollte als veränderbarer Vorschlag gekennzeichnet bleiben und darf keine ungeklärte Anforderung ersetzen.
...
\subsection{Kontrollierte Integration eines KI-gestützten Assistants in GitLab}
...
\begin{enumerate}[leftmargin=1.2cm]
\item \textbf{Aktivierung und Ereignisfilter.} GitLab-Webhooks liefern Issue- oder Note-Ereignisse. Parser, Bot-Filter, Mention-Erkennung, die Auswertung von Kommandos und Deduplication bestimmen, ob eine Verarbeitung stattfinden darf.
\item \textbf{Kontextaufbereitung.} Issue, Kommentare und optionaler Kontext aus dem Repository werden gesammelt, gekürzt und als Daten für den Analyseprompt aufbereitet. Fehlende externe Daten führen nach Möglichkeit zu einem kontrollierten Fallback.
\item \textbf{Gekapselter Modellaufruf.} Ein definierter Provider beziehungsweise Agent erhält zentrale Regeln und den vorbereiteten Kontext. Das Modell bekommt keine unmittelbare Berechtigung zur Änderung des Issues.
\item \textbf{Deterministische Antwortgrenze.} Die Ausgabe muss JSON mit exakt festgelegten Feldern enthalten. Parsing und Laufzeitvalidierung trennen probabilistische Textgenerierung von der weiteren Anwendung.
\item \textbf{Vorschau, Rückfrage und Persistenz.} Je nach Informationslage veröffentlicht die Anwendung einen Klärungskommentar oder einen strukturierten Vorschlag und speichert den Kontext zusätzlich lokal.
\item \textbf{Explizite Freigabe.} Ein zweiter, bewusster Publish-Schritt liest den gespeicherten Vorschlag, bereitet Titel und Beschreibung auf und aktualisiert erst dann das GitLab-Issue.
\end{enumerate}
...
\subsection{Übergreifende Antwort}
...
\section{Beitrag und Grenzen der Arbeit}

Die Arbeit liefert vier zusammenhängende Beiträge. Erstens wurde aus Literatur zu Requirements Engineering, User Stories und Bug Reports ein Qualitätsverständnis für GitLab-Tickets abgeleitet. Zweitens wurde daraus eine Architektur für einen ereignisgesteuerten, KI-gestützten und menschlich kontrollierten Ticketworkflow entwickelt. Drittens liegt mit Dev-Assist eine ausführbare Referenzimplementierung vor. Viertens macht die Evaluation nicht nur bestandene Tests, sondern auch erkannte und behobene Vertragsabweichungen, verbleibende Validierungslücken und Grenzen der Aussagekraft transparent.

Gerade die im Ausgangsstand festgestellten Abweichungen sind ein relevantes Ergebnis. Sie zeigen, dass Modularität und automatisierte Tests allein externe Verträge nicht garantieren. Die frühere interne HMAC-Testlogik konnte bestehen, obwohl sie nicht dem aktuellen GitLab-Verfahren entsprach; die überarbeitete Implementierung leitet ihre Prüfschritte deshalb unmittelbar aus dem dokumentierten Vertrag ab. Ebenso kann ein striktes JSON-Schema formal zuverlässig sein und dennoch eine gegenüber dem Anforderungsmodell reduzierte Struktur absichern. Qualitätssicherung muss Implementierung, Anforderungen, externe Spezifikationen und semantische Wirkung gemeinsam betrachten.
...
\section{Priorisierter Ausblick}
...
\subsection{Priorität 1: Vertragskorrektheit und belastbare Evaluation}

Die Webhook-Authentifizierung ist an den aktuellen GitLab-Signing-Token-Vertrag angepasst, mit dokumentationsnahen Payloads geprüft und standardmäßig verpflichtend (vgl. GitLab Docs, o. J.a, Abschnitt "Signing tokens"). Auch die Aktivierungsregel ist entsprechend FA-1 auf den Anfang der ersten Inhaltszeile begrenzt. Als nächster Schritt muss ein realer GitLab-Smoke-Test diese lokalen Vertragstests ergänzen. Für das Analyseformat sollte zudem eine einzige formale Schemaquelle entstehen, aus der TypeScript-Typen, Laufzeitvalidierung, Formattertests und Dokumentation abgeleitet werden.
...
\subsection{Bessere Metriken zur Ticketqualität}
...
\item \textbf{Strukturelle Vollständigkeit:} Sind Ziel, Kontext, Akzeptanzkriterien, technische Evidenz und offene Fragen vorhanden?
\item \textbf{Faktentreue:} Lässt sich jede konkrete Aussage auf Issue, Kommentar, Log oder Repository-Kontext zurückführen?
\item \textbf{Qualität der Rückfragen:} Sind Fragen relevant, nicht redundant, beantwortbar und auf entscheidende Lücken gerichtet?
\item \textbf{Prüfbarkeit:} Sind Akzeptanzkriterien beobachtbar, eindeutig und mit vertretbarem Aufwand testbar?
\item \textbf{Nutzbarkeit:} Können Entwicklerinnen und Entwickler das Ticket verstehen und eine Umsetzung planen, ohne wesentliche Informationen erneut zu suchen?
\item \textbf{Prozessaufwand:} Wie viel Zeit, Korrekturarbeit und zusätzliche Kommunikation entstehen im Vergleich zur manuellen Aufbereitung?
\end{itemize}
...
\subsection{Priorität 2: Produktiver Betrieb, Monitoring und Governance}
...
\subsection{Priorität 3: Funktionale Erweiterungen}
