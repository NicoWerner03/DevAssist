...
\midrule
\endhead
HTTP- und Routing-Schicht & Requests, JSON-Parsing, Rohkörper-Erfassung, Logging, Antwort an GitLab & \texttt{src/app.ts}, \texttt{src/server.ts}, \texttt{src/routes/gitlabWebhooks.ts}, \texttt{src/routes/issues.ts}, \texttt{src/routes/health.ts} \\
Webhook-Prüfung & Signatur-/Tokenprüfung, Payload-Parsing, Mention- und Kommandoerkennung, Deduplication & \texttt{src/services/gitlab/auth.ts}, \texttt{src/services/gitlab/parser.ts}, \texttt{src/services/gitlab/mention.ts}, \texttt{src/services/gitlab/commands.ts} \\
GitLab-Anbindung & Lesen von Issues und Kommentaren, Schreiben und Löschen von Notizen, Aktualisieren von Issues & \texttt{src/services/gitlab/client.ts}, \texttt{src/services/gitlab/cleanup.ts} \\
Analyse- und Agentenschicht & Prompt-Aufbau, Opencode- oder Mock-Analyse, JSON-Parsing, Schema-Prüfung, Rückfragen & \texttt{src/services/ai/service.ts}, \texttt{src/services/ai/instructions.ts}, \texttt{src/services/ai/schema.ts}, \texttt{src/services/ai/clarifications.ts}, \texttt{opencode.json} \\
Ausgabe- und Persistenzschicht & GitLab-Markdown, Kontextdateien & \texttt{src/services/ai/formatter.ts}, \texttt{src/services/context/writer.ts}, \texttt{src/services/context/reader.ts} \\
Prozesssteuerung & Orchestrierung von Analyse und Publish & \texttt{src/services/processing/processor.ts}, \texttt{src/services/processing/publisher.ts} \\
\bottomrule
\end{longtable}
...
Die Schema-Komponente definiert die erwartete Antwortstruktur mit Feldern wie \texttt{summary}, \texttt{sourceBasis}, \texttt{implementationTicket}, \texttt{acceptanceCriteria}, \texttt{technicalNotes}, \texttt{openQuestions}, \texttt{risks} und \texttt{validationSteps}. Erst diese maschinenlesbare Struktur ermöglicht deterministische Weiterverarbeitung. Der Formatter rendert gültige Analysen als GitLab-Markdown, entweder als Rückfragekommentar oder als vollständigen strukturierten Vorschlag. Context Writer und Reader speichern den Vorschlag unter \texttt{.dev-assist/issues/\textless{}projectId\textgreater{}/\textless{}issueIid\textgreater{}/context.md} und optionale Metadaten in \texttt{context.json}. Der Publisher liest diese Dateien, bereinigt Dev-Assist-bezogene Kommentare und aktualisiert anschließend Titel und Beschreibung des Issues.
...
Die Publish-Phase beginnt mit einem neuen GitLab-Kommentar oder einem manuellen API-Aufruf. Der Command-Parser erkennt \texttt{publish}, der Publisher liest \texttt{context.md} und \texttt{context.json}, bereitet die finale Issue-Beschreibung auf, extrahiert den vorgeschlagenen Titel und bereinigt Dev-Assist-bezogene Notizen. Anschließend aktualisiert er das Issue über die GitLab Issues API, die Titel, Beschreibung und weitere Felder ändern kann (vgl. GitLab Docs, o. J.c, Abschnitt "Update an issue"). Damit schreibt nicht das Modell produktiv nach GitLab, sondern die Anwendung nach expliziter menschlicher Freigabe.
...
\texttt{context.md} enthält den strukturierten Vorschlag als Markdown, unter anderem Zusammenfassung, Implementation Ticket, Scope, User Stories, funktionale Anforderungen, technische Hinweise, Definition of Done, Akzeptanzkriterien, offene Fragen, Risiken und Validierungsschritte. \texttt{context.json} enthält optionale Metadaten wie den vorgeschlagenen Titel. Markdown bleibt für Menschen lesbar, JSON für gezielte maschinelle Verarbeitung geeignet.
...
Deduplication schützt gegen Mehrfachverarbeitung. Da Webhooks wiederholt eintreffen oder erneut aus der Ereignishistorie gesendet werden können (vgl. GitLab Docs, o. J.a, Abschnitt "Manage webhooks"), speichert Dev-Assist für eine konfigurierbare Zeit Schlüssel aus Projekt, Issue, Ereignistyp und Ereignis-ID. Wiederholte Ereignisse erhalten einen erfolgreichen 2xx-Status, werden aber nicht erneut verarbeitet. Weitere Robustheit entsteht durch Konfiguration über Umgebungsvariablen, etwa für Port, Mention, GitLab-URL, \texttt{glab}, Token, Signaturpflicht, KI-Provider, Timeout, Kontextverzeichnis und Deduplication-TTL.
...
\midrule
\endhead
\texttt{src/server.ts}, \texttt{src/app.ts} & Serverstart, Middleware, Routen und Shutdown-Verhalten \\
\texttt{src/config.ts} & Einlesen und Normalisieren von Umgebungsvariablen \\
\texttt{src/routes/} & Health-Check, GitLab-Webhooks und manuelle Issue-Aktionen \\
\texttt{src/services/gitlab/} & Webhook-Parsing, Mention-Erkennung, Authentifizierung, GitLab-Client und Kommentarbereinigung \\
\texttt{src/services/ai/} & Promptaufbau, Mock- und OpenCode-Analyse, Antwortschema, Formatierung und Klärungslogik \\
\texttt{src/services/context/} & Schreiben und Lesen von \texttt{context.md} und \texttt{context.json} \\
\texttt{src/services/processing/} & Orchestrierung von Analyse und Publish \\
\texttt{src/services/repositorySummary.ts} & Sammlung und Zusammenfassung von Repository-Kontext \\
\texttt{tests/} & Automatisierte Tests für deterministische Systemteile \\
\bottomrule
\end{longtable}

Die Abhängigkeiten entsprechen dem Prototypumfang. \texttt{express} bildet den HTTP-Server, \texttt{dotenv} lädt die \texttt{.env}, \texttt{typescript}, \texttt{tsx} und Node-Typdefinitionen unterstützen Entwicklung und Tests. Die reale KI-Integration erfolgt derzeit über die OpenCode-CLI, obwohl \texttt{@opencode-ai/sdk} als Abhängigkeit vorhanden ist. Maßgeblich ist damit der CLI-Pfad: Die Anwendung startet \texttt{opencode run}, liest dessen Ausgabe und extrahiert daraus die strukturierte Analyse.

\section{Konfiguration, Startverhalten und Logging}

Die Laufzeitkonfiguration wird zentral in \texttt{src/config.ts} aufgebaut. Das Modul lädt die \texttt{.env} aus dem aktuellen Arbeitsverzeichnis und stellt eine typisierte \texttt{Config}-Struktur bereit. Wichtige Einstellungen sind Port, Log-Level, Mention-String, Bot-Benutzername, GitLab-Basis-URL, GitLab-Authentifizierungsmodus, Webhook-Signing-Secret, KI-Provider, Modell, Timeout, Kontextverzeichnis und Deduplication-Zeitfenster.
...
\midrule
\endhead
\texttt{/health} & einfacher Health-Endpunkt mit Status, Servicename und Zeitstempel \\
\texttt{/webhooks/gitlab/issues} & primärer Webhook-Endpunkt für Issue- und Kommentarereignisse \\
\texttt{/api/issues/:projectId/:issueIid/process}, \texttt{/api/issues/:projectId/:issueIid/publish} & manuelle Analyse- und Publish-Endpunkte \\
\bottomrule
\end{longtable}

Die manuellen Issue-Routen in \texttt{src/routes/issues.ts} verwenden dieselbe Prozess- und Publish-Logik wie der Webhook-Pfad. Ein Process-Aufruf startet \texttt{processIssue}, ein Publish-Aufruf startet \texttt{publishIssue} und kann zusätzliche GitLab-Felder wie \texttt{state\_event} oder Labels aus dem Request-Body übernehmen.
...
Zunächst ruft die Route \texttt{verifyWebhookRequest} aus \texttt{src/services/gitlab/auth.ts} auf. Ist kein Signing Secret konfiguriert, akzeptiert der Prototyp die Anfrage im Entwicklungsmodus und protokolliert eine Warnung. Für produktive GitLab-Webhook-Authentifizierung ist das Signing-Token-Verfahren relevant: GitLab sendet \texttt{webhook-id}, \texttt{webhook-timestamp} und bei konfiguriertem Signing Token \texttt{webhook-signature}; die Signatur wird über \texttt{\{message\_id\}.\{timestamp\}.\{body\}} aus Message-ID, Zeitstempel und rohem JSON-Body berechnet. Der Zeitstempel sollte auf Aktualität geprüft werden, um Replay-Angriffe zu erschweren. Falls \texttt{GITLAB\_REQUIRE\_SIGNATURE=true} gesetzt ist, führt eine fehlgeschlagene Prüfung zu \texttt{401 Unauthorized}; andernfalls wird im Entwicklungsmodus weiterverarbeitet. GitLab empfiehlt Signing Tokens, da sie Authentizität und Integrität per HMAC-SHA256 besser absichern als ein reiner Secret-Token-Header. Für ältere Installationen bleibt \texttt{X-Gitlab-Token} relevant, da GitLab Signing Tokens als in GitLab 19.0 eingeführt und in GitLab 19.1 allgemein verfügbar dokumentiert (vgl. GitLab Docs, o. J.a, Abschnitte "Create a webhook", "Signing tokens" und "Delivery headers").
...
\texttt{src/services/gitlab/commands.ts} bestimmt anschließend das Kommando. Beginnt der Text nach Entfernen der Mention mit \texttt{publish}, wird das Ereignis als Publish-Kommando interpretiert; andernfalls startet eine Analyseanforderung. Zusätzlich verhindert eine im Arbeitsspeicher gehaltene Deduplication Mehrfachverarbeitung. Sie speichert für ein konfigurierbares Zeitfenster einen Schlüssel aus Projekt-ID, Issue-IID, Ereignisart und Ereignis-ID beziehungsweise Objekt-ID. Wiederholte Ereignisse werden mit \texttt{202 Accepted} beantwortet, aber nicht erneut verarbeitet. Für mehrere Instanzen oder Neustarts wäre eine persistente oder verteilte Deduplication erforderlich.
...
Unabhängig vom Erfolg des GitLab-Kommentars versucht die Anwendung, den strukturierten Kontext lokal zu persistieren. \texttt{writeContextFile} schreibt \texttt{.dev-assist/issues/\textless{}projectId\textgreater{}/\textless{}issueIid\textgreater{}/context.md} und speichert, sofern vorhanden, den vorgeschlagenen Titel in \texttt{context.json}. \texttt{processFromWebhook} verbindet Parser und Prozesslogik, liest Projekt-ID, Issue-IID und auslösenden Text aus dem \texttt{ParsedWebhook}, prüft die Mention defensiv erneut und ruft \texttt{processIssue} auf.
...
Die OpenCode-Konfiguration steht in \texttt{opencode.json}. Dort sind \texttt{dev-assist-analyzer} für die Ticketanalyse und \texttt{repo-summary} für Repository-Zusammenfassungen definiert. Basisprompts liegen in \texttt{.opencode/prompts/requirement-analysis.md} und \texttt{.opencode/prompts/repo-summary.md}; der umfangreiche Analyseprompt wird jedoch zentral in \texttt{src/services/ai/instructions.ts} gepflegt. Dieses Modul enthält Persona, Kernregeln, JSON-Schema-Beispiel, Befüllungsregeln und Klärungshinweise.
...
Im OpenCode-Pfad kopiert \texttt{analyzeWithOpencode} erforderliche Dateien nach \texttt{.opencode/runtime}, startet \texttt{opencode run} mit \texttt{--format json}, \texttt{--agent dev-assist-analyzer}, dem konfigurierten Modell und dem vollständigen Prompt, sammelt \texttt{stdout} und \texttt{stderr}, setzt einen Timeout und beendet zu lange laufende Prozesse. Die OpenCode-CLI-Dokumentation beschreibt \texttt{opencode run}, Agent-Auswahl, Formatoptionen und Exportfunktionen als CLI-Funktionen (vgl. OpenCode Docs, o. J.d, o. S.).
...
Die Struktur der KI-Antwort ist in \texttt{src/services/ai/schema.ts} definiert. \texttt{RequirementAnalysis} enthält \texttt{summary}, \texttt{sourceBasis}, \texttt{implementationTicket}, \texttt{acceptanceCriteria}, \texttt{technicalNotes}, \texttt{openQuestions}, \texttt{risks} und \texttt{validationSteps}. Das verschachtelte \texttt{implementationTicket} enthält Titel, Ziel, Scope, Out-of-Scope, User Stories, funktionale Anforderungen, technischen Ansatz, Umsetzungsschritte und Definition of Done.
...
Die Laufzeitvalidierung erfolgt über \texttt{parseAnalysisJson} und \texttt{validateRequirementAnalysis}. Zunächst werden mögliche Markdown-Codezäune entfernt, dann wird JSON geparst. Anschließend prüft \texttt{validateRequirementAnalysis}, ob alle Pflichtfelder und die Pflichtfelder des verschachtelten \texttt{implementationTicket} vorhanden sind. Diese Validierung ist bewusst einfach und ohne zusätzliche Bibliothek umgesetzt. Sie prüft die Grundstruktur, aber nicht jedes Array-Element und jeden Feldtyp so streng wie eine vollständige JSON-Schema- oder Zod-Validierung.
...
Die Formatierung übernimmt \texttt{src/services/ai/formatter.ts}. \texttt{renderClarificationComment} erstellt Rückfragekommentare, wenn wichtige Informationen fehlen. \texttt{renderRequirementAnalysis} erzeugt das vollständige Kontextdokument mit Abschnitten wie Summary, Goal, Scope, User Stories, Functional Requirements, Technical Approach, Acceptance Criteria, Open Questions, Risks and Assumptions und Validation Steps. Schwache Listenwerte wie "not specified", "to be confirmed" oder "unknown" werden durch "Not enough information available yet." ersetzt, damit unsichere Informationen nicht scheinbar konkret erscheinen. Dies passt zum Umgang mit Halluzinationsrisiken: NIST beschreibt Confabulation beziehungsweise Hallucination als überzeugend präsentierte, aber falsche oder irreführende Inhalte (vgl. National Institute of Standards and Technology, 2024, S. 4 und S. 6).

\section{Umgang mit Rückfragen und Kommentarverlauf}

Ein wichtiges Implementierungsdetail ist die Behandlung bereits beantworteter Rückfragen. Ohne diese Logik könnte das Modell bei erneuter Analyse dieselben Fragen wiederholen, obwohl sie im Kommentarverlauf bereits beantwortet wurden. \texttt{src/services/ai/clarifications.ts} adressiert dieses Problem.
...
Die Persistenz liegt in \texttt{src/services/context/writer.ts} und \texttt{src/services/context/reader.ts}. \texttt{writeContextFile} erzeugt \texttt{.dev-assist/issues/\textless{}projectId\textgreater{}/\textless{}issueIid\textgreater{}/}, schreibt dort \texttt{context.md} und optional \texttt{context.json}. \texttt{context.md} enthält den gerenderten Vorschlag, \texttt{context.json} derzeit vor allem den vorgeschlagenen Titel. Beim Publish liest \texttt{readContextFile} den Markdown-Inhalt und \texttt{readContextMetadata} die Metadaten. Fehlt die Metadatendatei, wird mit der Beschreibung allein weitergearbeitet.
...
\midrule
\endhead
\texttt{tests/gitlab.test.ts} & Parsing von Issue- und Note-Events, Publish-Kommando, eigene und generierte Kommentare \\
\texttt{tests/auth.test.ts} & Verhalten ohne Secret, fehlende, gültige und ungültige HMAC-Signaturen \\
\texttt{tests/cleanup.test.ts} & Auswahl löschbarer und nicht löschbarer Kommentare \\
\texttt{tests/formatter.test.ts} & Rückfragekommentare, Kontextdokumente und Platzhalter für schwache Abschnitte \\
\texttt{tests/aiPrompt.test.ts} & Repository-Summary, beantwortete Rückfragen und Filterung offener Fragen \\
\texttt{tests/aiOpencode.test.ts} & Parsing über OpenCode-Export-Fallback \\
\texttt{tests/repositorySummary.test.ts} & Sammlung, Caching und Extraktion von Repository-Zusammenfassungen \\
\texttt{tests/config.test.ts}, \texttt{tests/tunnel.test.ts} & Konfigurationsdetails und Cloudflare-Tunnel-Helfer \\
\bottomrule
\end{longtable}
...
Projektinterne Arbeitsgrundlagen: \path{README.md}, \path{descriptions/project_description.txt}, \path{AGENTS.md}, \path{package.json}, \path{tsconfig.json}, \path{opencode.json}, \path{src/server.ts}, \path{src/app.ts}, \path{src/config.ts}, \path{src/routes/gitlabWebhooks.ts}, \path{src/routes/issues.ts}, \path{src/routes/health.ts}, \path{src/services/gitlab/parser.ts}, \path{src/services/gitlab/mention.ts}, \path{src/services/gitlab/commands.ts}, \path{src/services/gitlab/auth.ts}, \path{src/services/gitlab/client.ts}, \path{src/services/gitlab/cleanup.ts}, \path{src/services/gitlab/glab.ts}, \path{src/services/processing/processor.ts}, \path{src/services/processing/publisher.ts}, \path{src/services/ai/service.ts}, \path{src/services/ai/instructions.ts}, \path{src/services/ai/schema.ts}, \path{src/services/ai/formatter.ts}, \path{src/services/ai/clarifications.ts}, \path{src/services/ai/opencodeRuntime.ts}, \path{src/services/context/writer.ts}, \path{src/services/context/reader.ts}, \path{src/services/repositorySummary.ts}, \path{src/services/tunnel.ts}, \path{src/utils/logger.ts}, \path{src/cli/publish-issue.ts}, \path{tests/aiOpencode.test.ts}, \path{tests/aiPrompt.test.ts}, \path{tests/auth.test.ts}, \path{tests/cleanup.test.ts}, \path{tests/config.test.ts}, \path{tests/formatter.test.ts}, \path{tests/gitlab.test.ts}, \path{tests/glab.test.ts}, \path{tests/opencodeRuntime.test.ts}, \path{tests/processor.test.ts}, \path{tests/publisher.test.ts}, \path{tests/repositorySummary.test.ts}, \path{tests/schema.test.ts}, \path{tests/tunnel.test.ts} und \path{tests/webhookRoute.test.ts}.
