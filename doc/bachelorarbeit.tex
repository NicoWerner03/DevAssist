...
\newcommand{\arbeitstitel}{Konzeption und Implementierung eines KI-gestützten GitLab-Assistenten zur Verbesserung der Qualität von Software-Tickets}
\newcommand{\studentname}{[VOR- UND NACHNAME ERGÄNZEN]}
\newcommand{\matrikelnummer}{[MATRIKELNUMMER ERGÄNZEN]}
\newcommand{\studiengang}{[STUDIENGANG UND KURS ERGÄNZEN]}
\newcommand{\dualpartner}{[DUALEN PARTNER ERGÄNZEN]}
\newcommand{\betrieblicherbetreuer}{[BETRIEBLICHE BETREUUNG ERGÄNZEN]}
\newcommand{\wissenschaftlicherbetreuer}{[WISSENSCHAFTLICHE BETREUUNG ERGÄNZEN]}
...
\begin{titlepage}
\centering
{\Large Duale Hochschule Baden-Württemberg\par}
\vspace{1.5cm}
{\LARGE\bfseries \arbeitstitel\par}
\vspace{1.2cm}
{\large Bachelorarbeit\par}
\vspace{0.4cm}
{im Studiengang \studiengang\par}
\vfill
\begin{tabular}{@{}p{0.34\textwidth}p{0.58\textwidth}@{}}
Verfasserin/Verfasser: & \studentname \\
Matrikelnummer: & \matrikelnummer \\
Dualer Partner: & \dualpartner \\
Betriebliche Betreuung: & \betrieblicherbetreuer \\
Wissenschaftliche Betreuung: & \wissenschaftlicherbetreuer \\
...
Software-Tickets bilden in vielen Entwicklungsteams die zentrale Schnittstelle zwischen fachlichem Bedarf, technischer Umsetzung und Qualitätssicherung. In der Praxis sind relevante Informationen jedoch häufig unvollständig, unstrukturiert oder über Beschreibung und Kommentare verteilt. Diese Arbeit untersucht deshalb, welche Angaben ein GitLab-Ticket für eine nachvollziehbare Umsetzung und Prüfung benötigt und wie ein KI-gestützter Assistent kontrolliert in einen GitLab-Workflow integriert werden kann.

Hierzu wird Dev-Assist als Proof of Concept konzipiert und in TypeScript mit Express umgesetzt. GitLab-Webhooks aktivieren die Verarbeitung; der Dienst sammelt Issue-, Kommentar- und optionalen Repository-Kontext, übergibt ihn an einen über OpenCode eingebundenen Analyseagenten und validiert die Rückgabe gegen einen festen Datenvertrag. Abhängig von der Informationslage stellt das System Rückfragen oder erzeugt einen strukturierten Vorschlag. Eine produktive Änderung des Issues erfolgt erst nach einem separaten Publish-Kommando. Signaturprüfung, Replay-Schutz, Deduplizierung, Bot-Schleifenschutz, lokale Kontextpersistenz und eine explizite Freigabe begrenzen dabei die Autonomie des Modells.

Die technische Evaluation umfasst den TypeScript-Build, 71 automatisierte Tests in 17 Suiten und eine Coverage-Erhebung. Sämtliche Tests bestehen; für die geladenen Quelldateien werden 84,10\,\% Zeilenabdeckung erreicht. Nachgewiesen sind damit vor allem deterministische Systemeigenschaften wie Parsing, Schema-Validierung, Formatierung und Sicherheitsgrenzen. Nicht nachgewiesen ist die fachliche Qualität realer Modellantworten im produktiven Einsatz. Die Arbeit kommt zu dem Ergebnis, dass generative KI die Anforderungsaufbereitung unterstützen kann, wenn ein deterministischer Anwendungskern, minimale Berechtigungen und menschliche Freigabe die Modellgenerierung begrenzen. Für eine belastbare Wirkungsaussage sind reale Ende-zu-Ende-Tests und eine empirische Bewertung mit einem kuratierten Ticketkorpus erforderlich.

\chapter*{Abstract}
\addcontentsline{toc}{chapter}{Abstract}
Software tickets are a central interface between business needs, technical implementation, and quality assurance in many development teams. In practice, relevant information is often incomplete, unstructured, or distributed across an issue description and subsequent comments. This thesis therefore investigates which information a GitLab ticket needs to support comprehensible implementation and verification, and how an AI-assisted service can be integrated into a GitLab workflow under explicit technical control.

To address these questions, Dev-Assist is designed and implemented as a TypeScript and Express proof of concept. GitLab webhooks activate the workflow; the service collects issue and comment data as well as optional repository context, submits the prepared input to an analysis agent through OpenCode, and validates the response against a fixed data contract. Depending on the available information, the service either asks focused clarification questions or produces a structured ticket proposal. The GitLab issue is modified only after a separate publish command. Signature verification, replay protection, deduplication, bot-loop prevention, local context persistence, and explicit approval limit the model's autonomy.

The technical evaluation comprises a successful TypeScript build, 71 automated tests in 17 suites, and a coverage run. All tests pass, and the loaded source files reach 84.10\,\% line coverage. This evidence primarily supports deterministic properties such as parsing, schema validation, formatting, and security boundaries. It does not demonstrate the semantic quality of real model responses in production. The thesis concludes that generative AI can support requirements refinement when model generation is enclosed by deterministic application logic, minimal privileges, and human approval. Reliable claims about practical effectiveness require real end-to-end GitLab tests and an empirical evaluation based on a curated ticket corpus and expert assessments.
...
Nach erfolgreicher Analyse entscheidet der Processor, welcher Kommentar gepostet wird. Bei mindestens zwei substantiellen offenen Fragen wird \texttt{renderClarificationComment} verwendet; andernfalls erzeugt \texttt{renderRequirementAnalysis} einen vollständigen strukturierten Vorschlag. Diese Heuristik verhindert, dass einzelne offene Punkte den gesamten Vorschlag blockieren, während mehrere relevante Fragen als Hinweis auf eine noch instabile Ticketgrundlage behandelt werden.
...
Der Publish-Schritt befindet sich in \texttt{src/services/processing/publisher.ts}. Zunächst wird der gespeicherte Kontext gelesen. Danach entfernt \texttt{renderPublishedDescription} Abschnitte, die nicht in die finale Issue-Beschreibung gehören, etwa den separaten Titelabschnitt. Der Titel wird bevorzugt aus \texttt{context.json} übernommen, andernfalls aus dem Markdown extrahiert. So landet er im GitLab-Issue-Titel und nicht redundant im Beschreibungstext.
...
GitLab-Webhooks können lokal per \texttt{curl} oder PowerShell an \texttt{/webhooks/gitlab/issues} gesendet werden. \texttt{README.md} enthält dafür ein Beispiel mit \texttt{object\_kind: "issue"}, Projekt-ID, Issue-IID, Titel und Beschreibung mit \texttt{@dev-assist}. Soll GitLab den lokalen Dienst erreichen, kann \texttt{src/services/tunnel.ts} mit Cloudflare Tunnel eine öffentliche \texttt{trycloudflare.com}-URL erzeugen und den Webhook-Pfad anhängen.
...
Projektinterne Arbeitsgrundlagen: \path{README.md}, \path{descriptions/project_description.txt}, \path{AGENTS.md}, \path{package.json}, \path{tsconfig.json}, \path{opencode.json}, \path{.github/workflows/ci.yml}, \path{src/server.ts}, \path{src/app.ts}, \path{src/config.ts}, \path{src/routes/gitlabWebhooks.ts}, \path{src/routes/issues.ts}, \path{src/routes/health.ts}, \path{src/services/gitlab/parser.ts}, \path{src/services/gitlab/mention.ts}, \path{src/services/gitlab/commands.ts}, \path{src/services/gitlab/auth.ts}, \path{src/services/gitlab/client.ts}, \path{src/services/gitlab/cleanup.ts}, \path{src/services/gitlab/glab.ts}, \path{src/services/http/auth.ts}, \path{src/services/processing/processor.ts}, \path{src/services/processing/publisher.ts}, \path{src/services/ai/service.ts}, \path{src/services/ai/instructions.ts}, \path{src/services/ai/schema.ts}, \path{src/services/ai/formatter.ts}, \path{src/services/ai/clarifications.ts}, \path{src/services/ai/opencodeRuntime.ts}, \path{src/services/context/path.ts}, \path{src/services/context/writer.ts}, \path{src/services/context/reader.ts}, \path{src/services/repositorySummary.ts}, \path{src/services/tunnel.ts}, \path{src/utils/logger.ts}, \path{src/cli/publish-issue.ts}, \path{tests/aiOpencode.test.ts}, \path{tests/aiPrompt.test.ts}, \path{tests/auth.test.ts}, \path{tests/cleanup.test.ts}, \path{tests/config.test.ts}, \path{tests/formatter.test.ts}, \path{tests/gitlab.test.ts}, \path{tests/glab.test.ts}, \path{tests/opencodeRuntime.test.ts}, \path{tests/processor.test.ts}, \path{tests/publisher.test.ts}, \path{tests/repositorySummary.test.ts}, \path{tests/schema.test.ts}, \path{tests/security.test.ts}, \path{tests/tunnel.test.ts} und \path{tests/webhookRoute.test.ts}.
